...
public class J3dREFRFactory {
	...
	//Note fader = true
	private static J3dRECODynInst makeJ3dRECODynInst(	REFR refr, RECO reco, MODL modl, boolean makePhys,
	...
	private static J3dRECOStatInst makeJ3dRECOActionInst(	REFR refr, RECO reco, MODL modl, boolean makePhys,
															MediaSources mediaSources) {
		if (modl != null) {
	...
	//Note fader = true
	private static J3dRECOStatInst makeJ3dRECOStatInst(	REFR refr, RECO reco, MODL modl, boolean makePhys,
														MediaSources mediaSources) {
		if (modl != null) {
	...
	public static Node makeJ3DReferFar(REFR refr, IRecordStore master, MediaSources mediaSources) {
		...
		if (refr.xesp != null
			&& CommonREFR.getParentEnable(refr, master) != BethRenderSettings.isFlipParentEnableDefault()) {
	...
	public static J3dRECOInst makeJ3DRefer(	REFR refr, boolean makePhys, IRecordStore master,
											MediaSources mediaSources) {
		// list of object tpes
		...
		if (baseRecord.getRecordType().equals("STAT")) {
			...
			SCOL scol = new SCOL(baseRecord);
			J3dRECOStatInst j3dinst = new J3dRECOStatInst(refr, false, makePhys);
			j3dinst.setJ3dRECOType(new J3dRECOTypeStatic(scol, scol.MODL.model, makePhys, mediaSources));
			return j3dinst;
			...
			// my ambient effects are too darkening and foggy
			...
			if (acti.MODL != null
				&& (!acti.isFlagSet(RECO.IsMarker_Flag) || BethRenderSettings.isShowEditorMarkers())) {
			...
			if (cont.MODL != null
				&& (!cont.isFlagSet(RECO.IsMarker_Flag) || BethRenderSettings.isShowEditorMarkers())) {
			...
			if (furn.MODL != null
				&& (!furn.isFlagSet(RECO.IsMarker_Flag) || BethRenderSettings.isShowEditorMarkers())) {
